...
- **Matte engine — local by default, HeyGen Bria opt-in:** matting defaults to the hyperframes CLI's local `remove-background` (u2net) — byte-stable, and the right default per the 5-model A/B (Bria isn't a universal win: it keeps mic booms that u2net drops). Set **`EC_MATTE=cloud`** to opt into the live HeyGen **Background Removal API** (real Bria GPU — sharper edges + fewer furniture leaks): `scripts/matte-cloud.cjs` calls `/v3/background-removals` directly over REST — the same self-contained pattern as `hyperframes-media`'s HeyGen TTS, **no separate `heygen` binary** (hyperframes doesn't bundle one). Needs a HeyGen key — `$HEYGEN_API_KEY` (or `$HYPERFRAMES_API_KEY`), or `hyperframes auth login` → shared `~/.heygen/credentials`. Any cloud failure (incl. missing key) falls back to local, so the opt-in never breaks a render. (Free at launch; per-second billing is planned.)
